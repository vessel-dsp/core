...
const DEFAULT_CRT_BACKGROUND: VesselPreviewCrtBackground = {
	enabled: false,
...
export const VESSEL_PREVIEW_EFFECT_DEFAULTS: ResolvedVesselPreviewEffectPreset = {
	schema: "vessel-preview-effects/v1",
	toon: false,
...
export function applyToonMaterials(
	root: THREE.Object3D,
	presetInput: VesselPreviewEffectPreset | ResolvedVesselPreviewEffectPreset,
): THREE.Object3D {
...
export function addToonOutlines(
	root: THREE.Object3D,
	presetInput: VesselPreviewEffectPreset | ResolvedVesselPreviewEffectPreset,
): THREE.Object3D {
	const preset = resolvePresetLike(presetInput);
	...
	const existing = new Set(
		root.children.flatMap((child) => outlineNamesFor(child)),
	);
	root.traverse((object) => {
...
function outlineNamesFor(object: THREE.Object3D): string[] {
	const names: string[] = [];
...
export function applyMaterialGrain(
	root: THREE.Object3D,
	presetInput: VesselPreviewEffectPreset | ResolvedVesselPreviewEffectPreset,
): THREE.Object3D {
	const preset = resolvePresetLike(presetInput);
...
export function createScreenGrainPass(
	presetInput: VesselPreviewEffectPreset | ResolvedVesselPreviewEffectPreset,
): VesselScreenGrainPass {
	const preset = resolvePresetLike(presetInput);
	return {
		kind: "vessel-screen-grain-pass",
...
export function createPreviewEffectPipeline(
	presetInput: VesselPreviewEffectPreset | ResolvedVesselPreviewEffectPreset,
	options: {
...
function resolvePresetLike(
	preset: VesselPreviewEffectPreset | ResolvedVesselPreviewEffectPreset,
): ResolvedVesselPreviewEffectPreset {
	return "toon" in preset &&
		"grain" in preset &&
		"glitch" in preset &&
		"reducedMotion" in preset
		? (preset as ResolvedVesselPreviewEffectPreset)
		: resolvePreviewEffectPreset(preset as VesselPreviewEffectPreset);
}
...
function materialsFor(mesh: THREE.Mesh): readonly THREE.Material[] {
...
function unitInterval(value: number | undefined, defaultValue: number): number {
...
function resolveCrtBackground(
	background: Partial<VesselPreviewCrtBackground> | undefined,
): VesselPreviewCrtBackground {
	return {
		enabled: background?.enabled ?? DEFAULT_CRT_BACKGROUND.enabled,
		backgroundColor:
			nonEmptyText(background?.backgroundColor) ??
			DEFAULT_CRT_BACKGROUND.backgroundColor,
		gridColor:
			nonEmptyText(background?.gridColor) ?? DEFAULT_CRT_BACKGROUND.gridColor,
		gridOpacity: unitInterval(
			background?.gridOpacity,
			DEFAULT_CRT_BACKGROUND.gridOpacity,
		),
		gridSizePx: positiveNumber(
			background?.gridSizePx,
			DEFAULT_CRT_BACKGROUND.gridSizePx,
		),
		gridLineWidthPx: positiveNumber(
			background?.gridLineWidthPx,
			DEFAULT_CRT_BACKGROUND.gridLineWidthPx,
		),
	};
}

function createPreviewCrtFragmentShader(
	fragmentShader: string,
	background: VesselPreviewCrtBackground,
): string {
