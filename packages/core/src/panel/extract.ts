...
export function extractDeviceInterface(
	doc: CircuitDocument,
): ExtractedDeviceInterface {
	...
	return {
		...
		groupMemberships: resolveControlGroupMemberships(
			doc.controlGroups ?? [],
			controls,
		),
...
function inferDeviceInterfaceControls(
	...
	for (const switchControl of panel.switches) {
		controls.push({
			...
			provenance: provenanceForComponentControl(
				doc,
				componentIdFromControlId(switchControl.id),
			),
		});
	}
	...
	for (const jack of panel.jacks) {
		const componentId =
			jack.sourceComponentId ?? componentIdFromControlId(jack.id);
...
function provenanceForComponentControl(
	doc: CircuitDocument,
	componentId: string,
): DeviceInterfaceProvenance {
...
function applyControlInterfaces(
	...
	for (const controlInterface of controlInterfaces ?? []) {
		...
		const existingIndex =
			controlInterface.componentId === undefined
				? -1
...
function toControlInterfaceJack(controlInterface: ControlInterface): JackPort {
	...
	const controlRole =
		controlInterface.controlRole ?? defaultControlRole(controlInterface);
	const interfaceName =
		controlInterface.interface ?? defaultInterfaceName(controlInterface);
	return {
...
function jackRoleForControlInterface(
...
function toKnob(component: Component): Knob {
	const taper = resolveTaper(
		propertyString(component, "Sweep") ?? propertyString(component, "Taper"),
	);
	const stepLabels = parseStepLabels(
		propertyStringAny(component, ["StepLabels", "Steps"]),
	);
	const explicitStepCount = parseStepCount(
		propertyStringAny(component, [
			"StepCount",
			"Detents",
			"Positions",
			"Steps",
	...
	const steps = buildKnobSteps(
		stepLabels.length >= 2 ? stepLabels.length : (explicitStepCount ?? 0),
		stepLabels,
	);
	const rawDefaultPosition = clamp01(
		parseNumeric(component.properties.Wipe) ?? 0.5,
	);
...
function toSlider(component: Component): SliderControl {
	const defaultPosition = clamp01(
		parseNumeric(component.properties.Wipe) ?? 0.5,
	);
	const orientation = resolveSliderOrientation(
		propertyStringAny(component, ["Orientation", "SliderOrientation"]),
	);
...
function toSwitch(component: Component): SwitchControl {
	...
	const defaultPosition = clampInt(
		parseNumeric(component.properties.Position) ?? 0,
		0,
		positions - 1,
	);
	const gangGroup = propertyString(component, "Group") ?? undefined;
...
function toJack(component: Component): JackPort {
	...
	const name =
		nonEmptyString(propertyStringAny(component, ["JackLabel", "Label"])) ??
		component.name;
	const audioRole = nonEmptyString(propertyString(component, "AudioRole")) as
		| JackAudioRole
		| undefined;
...
function runtimeDescriptorKnobs(component: Component): readonly Knob[] {
	...
	for (const spec of RUNTIME_CONTINUOUS_CONTROL_SPECS) {
		const name = nonEmptyString(
			propertyString(component, spec.controlProperty),
		);
		if (name === undefined) {
			continue;
		}
		...
		knobs.push({
			...
			defaultPosition: clamp01(
				parseNumeric(component.properties[spec.wipeProperty]) ?? 0.5,
			),
		});
	}
...
function runtimeDescriptorMode(component: Component): Knob | undefined {
	...
	const steps = buildKnobSteps(
		labels.length >= 2 ? labels.length : (explicitStepCount ?? 0),
		labels,
	);
...
function runtimeDescriptorDirectOut(
	component: Component,
): JackPort | undefined {
	const name = nonEmptyString(
		propertyStringAny(component, [
...
function isVariableResistorControl(component: Component): boolean {
	return (
		component.properties.Wipe !== undefined ||
...
function switchGeometry(kind: SwitchKind): {
	poles: number;
	positions: number;
} {
	switch (kind) {
...
function normalizeJackRole(value: string): JackRole | null {
	...
	if (
		["direct-output", "direct-out", "dry-output", "dry-out"].includes(
			normalized,
		)
	)
	...
	if (["expression", "exp", "expression-pedal"].includes(normalized))
		return "expression";
	if (
		["tempo-tap", "tap-tempo", "tempo-in", "tap", "tempo"].includes(normalized)
	)
		return "tempo-tap";
	if (
		[
...
function shortType(sourceTypeName: string | null): string | null {
	...
	const match = sourceTypeName.match(
		/Circuit\.(?:Components\.)?([A-Za-z0-9_]+)/,
	);
...
function runtimeModeDefaultPosition(
