...
export function findPanelControl(
	panel: Panel,
	controlId: string,
): ControlUiControlRef | undefined {
	const knob = panel.knobs.find((control) => control.id === controlId);
	if (knob !== undefined) {
		return { kind: "knob", control: knob };
	}

	const slider = (panel.sliders ?? []).find(
	...
	const switchControl = panel.switches.find(
		(control) => control.id === controlId,
	);
...
export function controlValueForId(
	state: ControlState,
	controlId: string,
...
export function snapControlPosition(
...
export function normalizeSwitchPosition(
	position: number,
	positions: number,
): number {
...
export function valueForControl(
	...
	switch (control.kind) {
		case "knob":
			return {
				kind: "knob",
				position: snapControlPosition(control, valuePosition(value)),
			} satisfies KnobValue;
		case "slider":
			return {
				kind: "slider",
				position: clampControlPosition(valuePosition(value)),
			} satisfies SliderValue;
		case "switch":
			return {
				kind: "switch",
				position: normalizeSwitchPosition(
					valuePosition(value),
					control.control.positions,
				),
		...
		case "led": {
			...
			throw new Error(
				`LED control "${control.control.id}" is read-only in control-ui`,
			);
		}
		case "jack":
			throw new Error(
				`Jack "${control.control.id}" is read-only in control-ui`,
			);
...
function formatSliderValue(
	slider: { range?: { min: number; max: number; unit?: string } },
	position: number,
): string {
	...
	const value =
		slider.range.min +
		clampControlPosition(position) * (slider.range.max - slider.range.min);
