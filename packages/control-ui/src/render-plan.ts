...
export function createControlSurfaceRenderPlan(
	...
	for (const face of panel.placement?.faces ?? []) {
		for (const element of face.elements) {
			...
			const control =
				mounts.get(controlId) ?? findPanelControl(panel, controlId);
			if (control === undefined) {
				continue;
			}
			const item = renderItemForControl(
				control,
				options.appearance?.[controlId],
				element,
				face.id,
			);
			if (item !== undefined) {
	...
	for (const control of fallbackControls(panel)) {
		...
		const item = renderItemForControl(
			control,
			options.appearance?.[control.control.id],
		);
		if (item !== undefined) {
			items.push(item);
			markSeen(seen, control);
 ...
 */
function mountGroupsByControlId(
	panel: Panel,
): ReadonlyMap<string, ControlUiControlRef> {
