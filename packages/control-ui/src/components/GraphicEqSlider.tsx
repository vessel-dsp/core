...
export function GraphicEqSlider({
    ...
    return (
        <ControlFrame label={label} readout={readout} disabled={disabled} className={className} classNames={classNames}>
            <div
                className={cx('vdsp-control-ui-slider-shell', control.orientation === 'vertical' && 'is-vertical')}
                data-orientation={control.orientation}
            >
                <input
                    className={cx('vdsp-control-ui-control', 'vdsp-control-ui-slider', classNames?.control)}
                    type="range"
                    min={0}
                    max={100}
                    step={1}
                    ...
                    data-orientation={control.orientation}
                    onChange={(event) => onPositionChange?.(Number(event.currentTarget.value) / 100)}
                />
            </div>
        </ControlFrame>
    );
