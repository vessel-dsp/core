...
export function KnobControl({
	...
	const [localPosition, setLocalPosition] = useState<number | undefined>(
		undefined,
	);
	const [isDragging, setIsDragging] = useState(false);
	const normalizedPosition = snapControlPosition(
		{ kind: "knob", control },
		position,
	);
	...
	const readout = formatControlValue(
		{ kind: "knob", control },
		{ kind: "knob", position: displayPosition },
	);
	...
	function updateDragPosition(
		...
		const hasRelativeMovement =
			movementX !== undefined && Number.isFinite(movementX);
		const nextPosition = hasRelativeMovement
			? currentDrag.position + movementX / KNOB_DRAG_PIXELS_PER_RANGE
			: currentDrag.startPosition +
				(clientX - currentDrag.startX) / KNOB_DRAG_PIXELS_PER_RANGE;
		const snapped = snapPosition(nextPosition);
		if (snapped === undefined) {
			return;
		}
	...
	function handlePointerDown(
		...
		const removeDocumentListeners = addKnobDragDocumentListeners(
			ownerDocument,
			{
				move: (clientX, movementX, pointerId) =>
					updateDragPosition(pointerId, clientX, movementX),
