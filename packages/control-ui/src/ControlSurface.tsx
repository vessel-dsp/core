...
import { createControlSurfaceRenderPlan, type ControlSurfaceRenderItem } from './render-plan';
...
export function ControlSurface({
    ...
    function emitControlValue(controlId: string, value: ControlValue | number | boolean): void {
        if (disabled) {
            return;
        }
    ...
    return (
        <div
            className={cx('vdsp-control-ui-surface', disabled && 'is-disabled', classNames?.root, className)}
        ...
        >
            {plan.map((item) => (
                ...
                >
                    {renderControlItem(item, state, disabled, classNames, emitControlValue)}
                </div>
            ))}
        </div>
    );
...
function renderControlItem(
    ...
    disabled: boolean,
    classNames: ControlSurfaceClassNames | undefined,
    emitControlValue: (controlId: string, value: ControlValue | number | boolean) => void,
) {
        ...
        if (item.appearance === 'detented-rotary-select') {
            return (
                <DetentedRotarySelect
                    ...
                    position={value?.kind === 'knob' ? value.position : item.control.control.defaultPosition}
                    disabled={disabled}
                    label={item.label}
                    classNames={frameClassNames}
                    onPositionChange={(position) => emitControlValue(item.controlId, position)}
                />
            );
        }
        return (
    ...
    if (item.control.kind === 'slider') {
        return (
            <GraphicEqSlider
                control={item.control.control}
                position={value?.kind === 'slider' ? value.position : item.control.control.defaultPosition}
                disabled={disabled}
                label={item.label}
                classNames={frameClassNames}
                onPositionChange={(position) => emitControlValue(item.controlId, position)}
        ...
        if (item.appearance === 'footswitch') {
            return (
                <FootswitchButton
                    ...
                    label={item.label}
                    classNames={frameClassNames}
                    onPressedChange={(pressed) => emitControlValue(item.controlId, pressed ? 1 : 0)}
                />
            );
        }
        if (item.appearance === 'toggle') {
            return (
                <ToggleSwitchControl
                    ...
                    label={item.label}
                    classNames={frameClassNames}
                    onPositionChange={(nextPosition) => emitControlValue(item.controlId, nextPosition)}
                />
            );
        }
        return (
