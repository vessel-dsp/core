...
describe("parseCircuitJsonDocument", () => {
	test("imports source components, quantities, ports, traces, and synthesized layout diagnostics", () => {
		const doc = parseCircuitJsonDocument(SIMPLE_RC_CIRCUIT_JSON, {
		...
		expect(byId.get("R1")?.terminals.map((terminal) => terminal.name)).toEqual([
			"a",
			"b",
		]);
		...
		expect(
			isParsedQuantity(capacitance) ? capacitance.value : null,
		).toBeCloseTo(47e-9);

		const connectivity = resolveConnectivity(doc);
		expect(
			getPinNode(connectivity, { componentId: "R1", terminalName: "b" }),
		).toBe(getPinNode(connectivity, { componentId: "C1", terminalName: "a" }));
		expect(
		...
		expect(doc.warnings.map((warning) => warning.code)).toContain(
			"circuit-json-layout-synthesized",
		);
	});
	...
	test("preserves available schematic component geometry instead of synthesizing layout", () => {
		...
		expect(
			doc.components.find((component) => component.id === "R1")?.origin,
		).toEqual({ x: 12, y: -4 });
		expect(doc.warnings.map((warning) => warning.code)).not.toContain(
			"circuit-json-layout-synthesized",
		);
	});

	test("imports VesselDSP source-property sidecars and can-cap source groups", () => {
		const doc = parseCircuitJsonDocument([
			{
				type: "source_group",
	...
	test("does not import can-cap source groups onto non-capacitor components", () => {
		const doc = parseCircuitJsonDocument([
			...
			},
			{
				type: "source_property_ignored_warning",
				source_property_ignored_warning_id:
					"vessel_dsp_property:source_component:R1:CanCapSection",
				source_component_id: "source_component:R1",
				property_name: "CanCapSection",
				message:
					'vessel-dsp-property-json:{"property":"CanCapSection","value":"A"}',
...
describe("validateCircuitJsonDocument", () => {
	test("reports schema failures without throwing", () => {
		const result = validateCircuitJsonDocument([
			{ type: "source_component", source_component_id: 7 },
		]);
