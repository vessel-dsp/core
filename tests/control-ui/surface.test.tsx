...
describe("ControlSurface", () => {
	test("renders a panel with appearance overrides and class hooks", () => {
		...
		expect(
			renderer.root.findAllByProps({ "data-vdsp-control-id": "bright" }),
		).toHaveLength(0);
		expect(
			renderer.root.findByProps({ "data-vdsp-control-id": "gain" }).props
				.className,
		).toContain("tw-select");
		expect(
			renderer.root.findByProps({ "data-vdsp-control-id": "mode" }).props
				.className,
		).toContain("tw-select");
		expect(
			renderer.root.findByProps({ "data-vdsp-control-id": "eq" }).props
				.className,
		).toContain("tw-control");
	});
	...
	test("does not emit messages while disabled", () => {
		...
		const switchButton = renderer.root.findByProps({
			"data-vdsp-control-id": "bypass",
		});
		act(() => {
		...
		function Harness() {
			...
			return (
				<ControlUiThemeProvider theme={{ controlColor: "#0f766e" }}>
					<button
						type="button"
						onClick={() => controls.setControlValue("mode", 0.9)}
					>
						set mode
					</button>
					<ControlSurface
	...
	test("renders every dial of a concentric mount and emits per tier", () => {
		...
		for (const id of ["bass", "mid", "treble"]) {
			expect(
				renderer.root.findByProps({ "data-vdsp-control-id": id }),
			).toBeDefined();
		}

		const treble = renderer.root.findByProps({
			"data-vdsp-control-id": "treble",
		});
		act(() => {
