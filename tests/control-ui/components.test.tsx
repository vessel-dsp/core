...
const CONTROL_UI_STYLES_URL = new URL('../../packages/control-ui/src/styles.css', import.meta.url);
...
describe('control-ui React primitives', () => {
    ...
    test('uses orange as the default primary control color and lighter labels', () => {
        ...
        const defaultThemeRule = cssRuleBody(styles, ':where(.vdsp-control-ui-theme, .vdsp-control-ui-surface)');
        const labelRule = cssRuleBody(styles, '.vdsp-control-ui-label');

        expect(defaultThemeRule).toContain('--vdsp-control-ui-accent-color: #f97316;');
        expect(labelRule).toContain('color: color-mix(in srgb, var(--vdsp-control-ui-text-color) 72%, white);');
    });

    test('advertises horizontal knob dragging through cursor styles', () => {
        const styles = readFileSync(CONTROL_UI_STYLES_URL, 'utf8');

        expect(cssRuleBody(styles, '.vdsp-control-ui-knob')).toContain('cursor: ew-resize;');
        expect(cssRuleBody(styles, '.vdsp-control-ui-knob:active')).toContain('cursor: ew-resize;');
        expect(cssRuleBody(styles, '.vdsp-control-ui-knob.is-dragging')).toContain('cursor: ew-resize;');
        expect(
            cssRuleBody(styles, '.vdsp-control-ui-is-knob-dragging,\n.vdsp-control-ui-is-knob-dragging *'),
    ...
    test('updates knob rotation immediately during pointer drag and locks the cursor', () => {
        ...
        const knob = () => renderer.root.findByProps({ 'data-vdsp-control-id': 'gain' });

        act(() => {
            knob().props.onPointerDown({ pointerId: 7, clientX: 100, clientY: 100, currentTarget });
        });
        ...
        act(() => {
            documentListeners.mousemove?.({ clientX: 100, clientY: 20, movementX: 0, movementY: -80, preventDefault: () => {} });
        });
        ...
        act(() => {
            documentListeners.mousemove?.({ clientX: 180, clientY: 20, movementX: 80, movementY: 0, preventDefault: () => {} });
        });
        expect(positions).toEqual([0.75]);
        expect(knob().props.style['--vdsp-control-ui-knob-progress']).toBe('225deg');
        expect(knob().props.style['--vdsp-control-ui-knob-rotation']).toBe('75deg');
    ...
    test('keeps a horizontally dragged knob at the clamped value after release', () => {
        ...
        act(() => {
            knob().props.onPointerDown({ pointerId: 8, clientX: 240, clientY: 100, currentTarget });
        });
        act(() => {
            documentListeners.mousemove?.({ clientX: 40, clientY: 100, movementX: -200, movementY: 0, preventDefault: () => {} });
        });
        expect(positions).toEqual([0]);
        expect(knob().props.style['--vdsp-control-ui-knob-progress']).toBe('0deg');
        expect(knob().props.style['--vdsp-control-ui-knob-rotation']).toBe('-150deg');
    ...
    test('renders footswitch, toggle, slider, select, and LED semantics', () => {
        const html = renderToStaticMarkup(
            <div>
                <FootswitchButton control={controlUiTestPanel.switches[0]} pressed={true} className="rounded-full" />
                <ToggleSwitchControl control={controlUiTestPanel.switches[1]} position={1} className="tw-toggle" />
                <GraphicEqSlider control={controlUiTestPanel.sliders?.[0]} position={0.75} className="tw-slider" />
                <DetentedRotarySelect control={controlUiTestPanel.knobs[1]} position={0.5} className="tw-select" />
                <LedIndicator control={controlUiTestPanel.leds[0]} value={{ kind: 'led', on: true, intensity: 0.6 }} />
            </div>,
        );
    ...
    test('removes native slider track borders', () => {
        ...
        const webkitTrackRule = cssRuleBody(styles, '.vdsp-control-ui-slider::-webkit-slider-runnable-track');
        const webkitThumbRule = cssRuleBody(styles, '.vdsp-control-ui-slider::-webkit-slider-thumb');
        ...
        expect(cssRuleBody(styles, '.vdsp-control-ui-slider::-moz-range-track')).toContain('border: 0;');
    });

    test('styles the toggle as a flat control without shadows', () => {
        const styles = readFileSync(CONTROL_UI_STYLES_URL, 'utf8');
        const toggleRule = cssRuleBody(styles, '.vdsp-control-ui-toggle');
        const checkedRule = cssRuleBody(styles, '.vdsp-control-ui-toggle.is-checked');
        const thumbRule = cssRuleBody(styles, '.vdsp-control-ui-toggle__thumb');

        expect(toggleRule).toContain('background: var(--vdsp-control-ui-control-color);');
        expect(toggleRule).toContain('box-shadow: none;');
        expect(checkedRule).toContain('background: var(--vdsp-control-ui-accent-color);');
        expect(thumbRule).toContain('background: var(--vdsp-control-ui-background-color);');
    ...
    test('renders footswitch toggle state without latching the physical press visual', () => {
        const html = renderToStaticMarkup(
            <FootswitchButton control={controlUiTestPanel.switches[0]} pressed={true} />,
        );
    ...
    test('shows footswitch press feedback only while the actuator is held', () => {
        ...
        const footswitch = () => renderer.root.findByProps({ 'data-vdsp-control-id': 'bypass' });
    ...
    test('styles the footswitch as a solid circular actuator with held border feedback', () => {
        ...
        expect(footswitchRule).toContain('border: 1px solid var(--vdsp-control-ui-border-color);');
        expect(footswitchRule).toContain('border-radius: 50%;');
        expect(footswitchRule).toContain('background: var(--vdsp-control-ui-control-color);');
    ...
    test('styles the select dropdown with square dark pure CSS controls', () => {
        ...
        expect(selectRule).toContain('background-color: var(--vdsp-control-ui-control-color);');
        expect(selectRule).toContain('color: var(--vdsp-control-ui-background-color);');
        expect(selectRule).toContain('background-image:');
        expect(optionRule).toContain('background-color: var(--vdsp-control-ui-control-color);');
        expect(optionRule).toContain('color: var(--vdsp-control-ui-background-color);');
    });
