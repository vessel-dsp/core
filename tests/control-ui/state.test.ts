...
describe("control-ui state helpers", () => {
	...
	test("applies only panel-valid messages and keeps LED controls read-only from UI values", () => {
		...
		expect(controlValueForId(nextState, "gain")).toEqual({
			kind: "knob",
			position: 0.75,
		});
		expect(() =>
		...
		expect(() =>
			controlMessageForValue(controlUiTestPanel, "status", true),
		).toThrow("read-only");
	...
	test("looks up controls, clamps positions, snaps stepped knobs, and formats values", () => {
		...
		expect(
			mode === undefined ? undefined : snapControlPosition(mode, 0.74),
		).toBe(0.5);
		expect(
			mode === undefined
				? undefined
				: formatControlValue(mode, { kind: "knob", position: 0.5 }),
		).toBe("Crunch");
		expect(
			eq === undefined
				? undefined
				: formatControlValue(eq, { kind: "slider", position: 0.75 }),
		).toBe("6 dB");
		expect(
			bright === undefined
				? undefined
				: formatControlValue(bright, { kind: "switch", position: 1 }),
		).toBe("Position 2");
	});
	...
	test("plans render order from panel placement and resolves default appearances", () => {
		...
		expect(plan.map((item) => item.controlId)).toEqual([
			"bypass",
			"gain",
			"mode",
			"eq",
		...
		expect(
			createControlSurfaceRenderPlan(controlUiTestPanel, {
				appearance: { gain: "hidden" },
			}),
		).not.toContainEqual(expect.objectContaining({ controlId: "gain" }));
		expect(
			resolveControlAppearance({
				kind: "switch",
	...
	test("exposes jacks as read-only controls and renders them in the plan", () => {
		...
		expect(
			jack === undefined ? undefined : resolveControlAppearance(jack),
		).toBe("jack");
		expect(() => controlMessageForValue(controlUiJackPanel, "in", 1)).toThrow(
			"read-only",
		);
		...
		expect(
			plan
				.filter((item) => item.appearance === "jack")
	...
	test("groups a concentric mount of N pots into one render item", () => {
		...
		const concentric = plan.filter(
			(item) => item.appearance === "concentric-knob",
		);
