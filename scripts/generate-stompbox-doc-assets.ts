...
type DemoAssetPreset = Readonly<{
	...
	state?: NonNullable<
		Parameters<typeof createStompboxPreviewGlbFromVdsp>[1]
	>["state"];
	outputPrefix: string;
...
function styleProfileById(id: string): StompboxStyleProfile {
	const profile = DEMO_PROFILES.styleProfiles.find(
...
function drillTemplateAppearanceForPreset(
	preset: DemoAssetPreset,
): StompboxAppearance {
...
for (const preset of presets) {
	...
	const drillLayout = createStompboxDrillLayoutFromVdsp(
		preset.source,
		commonOptions,
	);
	const drillTemplateSvg = createStompboxDrillTemplateSvgFromVdsp(
		preset.source,
		{
			...commonOptions,
	...
	writeFileSync(
		join(DOC_EXAMPLES_DIR, `${preset.outputPrefix}-preview.glb`),
		glb.bytes,
	);
	writeFileSync(
	...
	writeFileSync(
		join(DOC_EXAMPLES_DIR, `${preset.outputPrefix}-drill-template-preview.svg`),
		drillTemplateSvg,
	);
	...
	if (preset.writeTopPreviewSvg === true) {
		...
		writeFileSync(
			join(DOC_EXAMPLES_DIR, `${preset.outputPrefix}-preview-top.svg`),
			views.views.top,
		);
	}
}
