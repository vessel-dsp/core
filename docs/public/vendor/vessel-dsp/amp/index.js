...
function addAmpGrilleNet(root, grille) {
    ...
    for (const direction of [-1, 1]) {
        for (const segment of clippedDiagonalSegments(grille.widthMm, grille.heightMm, AMP_GRILLE_NET_SPACING_MM, direction)) {
            const line = new THREE.Mesh(new THREE.BoxGeometry(segment.lengthMm, AMP_GRILLE_NET_STROKE_MM, AMP_GRILLE_NET_DEPTH_MM), lineMaterial);
...
function addAmpLabels(root, layout) {
    ...
    const brand = vectorTextLabel("amp-brand-label", layout.brandName, layout.appearance.brandLabelColor, layout.appearance.brandLabelFontSizeMm, layout.appearance.labelFontFamily, 2, { outlineColor: "#000000" });
    ...
    const model = vectorTextLabel("amp-model-label", layout.modelName, layout.appearance.modelLabelColor, layout.appearance.modelLabelFontSizeMm, layout.appearance.labelFontFamily, 2, { outlineColor: "#000000" });
...
function clippedDiagonalSegments(widthMm, heightMm, spacingMm, direction, clearBands = []) {
...
function subtractClearBands(extents, pointY, directionY, clearBands) {
