...
The circuit/PCB model is packaging-agnostic: two pots are two pots. A stacked
concentric pot is an **optional part-variant choice**, not a circuit property.
By default a layout auto-generates separate pots; concentric realization happens
only when the placement selects a multi-surface concentric part.
...
Stompbox groups elements by `mountId` into a **single drill hole** at the shared
...
Use [`@vessel-dsp/control-ui`](/core/guides/control-ui/) when the host app wants
...
Pass `crt={true}` to apply a full-screen CRT post-processing pass over both the
...
[gingerbeardman/webgl-crt-shader](https://github.com/gingerbeardman/webgl-crt-shader)
(MIT).
...
[`DigitalGlitch`](https://github.com/mrdoob/three.js/blob/master/examples/jsm/shaders/DigitalGlitch.js)
(MIT).
...
>
	...
	<StompboxGlbViewer src="/core/examples/stompbox-mxr-style-preview.glb" view="top" interactive={false} height="520px" backgroundColor="#091833" gridColor="#cccccc" gridOpacity={0.1} toon={true} toonEdgeColor="#69145a" linework={true} lineworkColor="#ffffff" grain={true} grainScale={1} grainIntensity={0.05} crt={true} />
