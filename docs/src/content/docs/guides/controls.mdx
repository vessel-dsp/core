...
when they need the extracted panel descriptor.
...
| `kind: "jack"` | `JackPort` |
| legacy/imported `kind: "ic"` plus `RuntimeDescriptor: "true"` | Compatibility descriptor knobs and external jack ports |
...
`ControlRole` is the machine-facing semantic role field for controls that a
...
`RuntimeDescriptor: "true"`. Core preserves them for `.schx` import and legacy
`.vdsp` compatibility, and `extractPanel()` can expose their source-visible
controls. Do not use descriptor fields as canonical `.vdsp` runtime authority;
host runtimes should derive lowering behavior from source facts plus their own
registries, overlays, or manifests.

Canonical `.vdsp` intake reports these as warnings from
`validateVdspCircuitDocumentSchema(source)`. Direct `CircuitDocument` callers
can use `validateSourceRuntimeBoundary(document)` or
`createSourceRuntimeBoundaryRule()` to report runtime/admission/proxy selectors
in component properties or top-level raw attributes, such as `RuntimeMatchKey`,
`RuntimeDescriptor`, `DescriptorType`, stored compiler certificate/admission
metadata, or legacy markers like `SourceOnly`, `InterfaceOnly`,
